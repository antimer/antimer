...
(defview view-article (slug)
  (flet ((render-error (message)
    ...
    (let ((article (antimer.db:find-article slug)))
      (if article
          ;; Find everything from the cache
          (let ((html
                  (antimer.cache:get-value (format nil "article.html.~A" slug)))
                (word-count
                  (parse-integer
                   (antimer.cache:get-value (format nil "article.wordcount.~A" slug)))))
            (render-template (+view-article+)
...
(defview file-data (filename)
  ;; When serving files, we don't care whether the file is actually registered
  ;; in the database. This allows us to serve autogenerated files.
  (let ((path (antimer.file:file-path filename)))
    (if (probe-file path)
...
(defmethod on-event ((plugin web-app) (event startup))
  ...
  (lucerne.app:use app
                   (make-instance 'clack.middleware.static:<clack-middleware-static>
                                  ...
                                  :path "/static/"))
  (start app :port (plugin-port plugin) :silent t))
